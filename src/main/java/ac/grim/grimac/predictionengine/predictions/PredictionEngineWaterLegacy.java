...
public class PredictionEngineWaterLegacy extends PredictionEngine {
    ...
    public MutableVector getMovementResultFromInput(GrimPlayer player, MutableVector inputVector, float f, float f2) {
        ...
        if (lengthSquared >= 1.0E-4F) {
            ...
            return new MutableVector(inputVector.getX() * cosResult - inputVector.getZ() * sinResult,
    ...
    public void addJumpsToPossibilities(GrimPlayer player, Set<VectorData> existingVelocities) {
        for (VectorData vector : new HashSet<>(existingVelocities)) {
            ...
            if (player.skippedTickInActualMovement) {
                existingVelocities.add(new VectorData(vector.vector.clone().add(new MutableVector(0, 0.02f, 0)), vector, VectorData.VectorType.Jump));
            }
        }
    }
    ...
    public void endOfTick(GrimPlayer player, double playerGravity) {
        ...
        for (VectorData vector : player.getPossibleVelocitiesMinusKnockback()) {
            vector.vector.multiply(new MutableVector(swimmingFriction, 0.8F, swimmingFriction));
