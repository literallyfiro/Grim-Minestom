...
public class PredictionEngine {
    ...
    public static MutableVector clampMovementToHardBorder(GrimPlayer player, MutableVector outputVel) {
        // TODO: Reimplement
        return outputVel;
    }

    public static MutableVector transformInputsToVector(GrimPlayer player, MutableVector theoreticalInput) {
        float bestPossibleX;
        float bestPossibleZ;
        ...
        MutableVector inputVector = new MutableVector(bestPossibleX, 0, bestPossibleZ);
        inputVector.multiply(0.98F);

        // Simulate float rounding imprecision
        inputVector = new MutableVector((float) inputVector.getX(), (float) inputVector.getY(), (float) inputVector.getZ());
        ...
        if (inputVector.lengthSquared() > 1) {
            ...
            inputVector = new MutableVector(inputVector.getX() / d0, inputVector.getY() / d0, inputVector.getZ() / d0);
        }
    ...
    public void guessBestMovement(float speed, GrimPlayer player) {
        ...
        if (player.uncertaintyHandler.influencedByBouncyBlock()) {
            for (VectorData data : init) {
                ...
                MutableVector toZeroVec = new PredictionEngine().handleStartingVelocityUncertainty(player, data, new MutableVector(0, -1000000000, 0)); // Downwards without overflow risk
    ...
    private void doPredictions(GrimPlayer player, List<VectorData> possibleVelocities, float speed) {
        ...
        MutableVector beforeCollisionMovement = null;
        MutableVector originalClientVel = player.clientVelocity.clone();
        ...
        for (VectorData clientVelAfterInput : possibleVelocities) {
            MutableVector primaryPushMovement = handleStartingVelocityUncertainty(player, clientVelAfterInput, player.actualMovement);

            MutableVector bestTheoreticalCollisionResult = VectorUtils.cutBoxToVector(player.actualMovement, new SimpleCollisionBox(0, Math.min(0, primaryPushMovement.getY()), 0, primaryPushMovement.getX(), Math.max(0.6, primaryPushMovement.getY()), primaryPushMovement.getZ()).sort());
            ...
            Pair<MutableVector, MutableVector> output = doSeekingWallCollisions(player, primaryPushMovement, originalClientVel, clientVelAfterInput);
            primaryPushMovement = output.getFirst();
            MutableVector outputVel = clampMovementToHardBorder(player, output.getSecond());
    ...
    private Pair<MutableVector, MutableVector> doSeekingWallCollisions(GrimPlayer player, MutableVector primaryPushMovement, MutableVector originalClientVel, VectorData clientVelAfterInput) {
        ...
        primaryPushMovement = new MutableVector(testX, testY, testZ);

        MutableVector outputVel = Collisions.collide(player, primaryPushMovement.getX(), primaryPushMovement.getY(), primaryPushMovement.getZ(), originalClientVel.getY(), clientVelAfterInput);
    ...
    private void addZeroPointThreeToPossibilities(float speed, GrimPlayer player, List<VectorData> possibleVelocities) {
        ...
        MutableVector pointThreeVector = new MutableVector();
        ...
        if (!player.pointThreeEstimator.controlsVerticalMovement()) {
            ...
            pointThreePossibilities.add(new VectorData(new MutableVector(0, player.clientVelocity.getY(), 0), VectorData.VectorType.ZeroPointZeroThree));
        }
        ...
        if (player.pointThreeEstimator.isNearFluid && !Collisions.isEmpty(player, player.boundingBox.copy().expand(0.4, 0, 0.4)) && !player.onGround) { // onGround can still be used here, else generic 0.03
            pointThreePossibilities.add(new VectorData(new MutableVector(0, 0.3, 0), VectorData.VectorType.ZeroPointZeroThree));
        }
        ...
        if (player.pointThreeEstimator.isNearClimbable() && (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_14) || !Collisions.isEmpty(player, player.boundingBox.copy().expand(
            ...
            MutableVector hackyClimbVector = new MutableVector(0, 0.2, 0);
        ...
        if (player.packetStateData.tryingToRiptide) {
            MutableVector riptideAddition = Riptide.getRiptideVelocity(player);
            pointThreePossibilities.add(new VectorData(player.clientVelocity.clone().add(riptideAddition), new VectorData(new MutableVector(), VectorData.VectorType.ZeroPointZeroThree), VectorData.VectorType.Trident));
        }
        ...
        for (VectorData vectorData : data) {
            ...
            if (vectorData.isKnockback() && player.baseTickWaterPushing.lengthSquared() != 0) {
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_13)) {
                    MutableVector vec3 = player.baseTickWaterPushing.clone();
    ...
    private void addAttackSlowToPossibilities(GrimPlayer player, Set<VectorData> velocities) {
        for (int x = 1; x <= Math.min(player.maxPlayerAttackSlow, 5); x++) {
            for (VectorData data : new HashSet<>(velocities)) {
                velocities.add(data.returnNewModified(data.vector.clone().multiply(new MutableVector(0.6, 1, 0.6)), VectorData.VectorType.AttackSlow));
            }
        }
    }
    ...
    public MutableVector handleStartingVelocityUncertainty(GrimPlayer player, VectorData vector, MutableVector targetVec) {
        ...
        //
        // Be somewhat careful as there is an antikb (for horizontal) that relies on this lenience
        MutableVector uncertainty = new MutableVector(avgColliding * 0.08, additionVertical, avgColliding * 0.08);

        MutableVector min = new MutableVector(player.uncertaintyHandler.xNegativeUncertainty - additionHorizontal, -bonusY + player.uncertaintyHandler.yNegativeUncertainty, player.uncertaintyHandler.zNegativeUncertainty - additionHorizontal);
        MutableVector max = new MutableVector(player.uncertaintyHandler.xPositiveUncertainty + additionHorizontal, bonusY + player.uncertaintyHandler.yPositiveUncertainty, player.uncertaintyHandler.zPositiveUncertainty + additionHorizontal);

        MutableVector minVector = vector.vector.clone().add(min.subtract(uncertainty));
        MutableVector maxVector = vector.vector.clone().add(max.add(uncertainty));
        ...
        if (!player.pointThreeEstimator.canPredictNextVerticalMovement()) {
            minVector.setY(minVector.getY() - player.compensatedEntities.getSelf().getAttributeValue(Attribute.GENERIC_GRAVITY));
        }
                ...
                for (VectorData possibleLastTickOutput : possibleVectors) {
                    ...
                    for (int x = -1; x <= 1; x++) {
                        for (int z = zMin; z <= 1; z++) {
                            VectorData result = new VectorData(possibleLastTickOutput.vector.clone().add(getMovementResultFromInput(player, transformInputsToVector(player, new MutableVector(x, 0, z)), speed, player.xRot)), possibleLastTickOutput, VectorData.VectorType.InputResult);
    ...
    public MutableVector getMovementResultFromInput(GrimPlayer player, MutableVector inputVector, float f, float f2) {
        ...
        return new MutableVector(xResult * f, 0, zResult * f);
    }
    ...
    public void doJump(GrimPlayer player, MutableVector vector) {
