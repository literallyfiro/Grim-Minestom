...
public class MovementTicker {
    ...
    public static void handleEntityCollisions(GrimPlayer player) {
        ...
        if (!player.compensatedEntities.getSelf().inVehicle()) {
            ...
            // todo minestom maybe  playerBox.expand(0.10);?
    ...
    public void move(MutableVector inputVel, MutableVector collide) {
        ...
        player.mainSupportingBlockData = MainSupportingBlockPosFinder.findMainSupportingBlockPos(player, player.mainSupportingBlockData, new Vec(collide.getX(), collide.getY(), collide.getZ()), player.boundingBox, player.onGround);
        Block onBlock = BlockProperties.getOnPos(player, player.mainSupportingBlockData, new Vec(player.x, player.y, player.z));
        ...
        float f = BlockProperties.getBlockSpeedFactor(player, player.mainSupportingBlockData, new Vec(player.x, player.y, player.z));
    ...
    public void livingEntityTravel() {
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                ...
                float blockFriction = BlockProperties.getFriction(player, player.mainSupportingBlockData, new Vec(player.lastX, player.lastY, player.lastZ));
