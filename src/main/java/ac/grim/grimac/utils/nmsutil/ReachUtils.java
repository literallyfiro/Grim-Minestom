...
public class ReachUtils {
    ...
    public static Pair<MutableVector, BlockFace> calculateIntercept(SimpleCollisionBox self, MutableVector origin, MutableVector end) {
        MutableVector minX = getIntermediateWithXValue(origin, end, self.minX);
        MutableVector maxX = getIntermediateWithXValue(origin, end, self.maxX);
        MutableVector minY = getIntermediateWithYValue(origin, end, self.minY);
        MutableVector maxY = getIntermediateWithYValue(origin, end, self.maxY);
        MutableVector minZ = getIntermediateWithZValue(origin, end, self.minZ);
        MutableVector maxZ = getIntermediateWithZValue(origin, end, self.maxZ);
     ...
     */
    public static MutableVector getIntermediateWithXValue(MutableVector self, MutableVector other, double x) {
        ...
        if (d0 * d0 < 1.0000000116860974E-7D) {
            ...
            return d3 >= 0.0D && d3 <= 1.0D ? new MutableVector(self.getX() + d0 * d3, self.getY() + d1 * d3, self.getZ() + d2 * d3) : null;
        }
    }
     ...
     * passed in vector, or null if not possible.
     */
    public static MutableVector getIntermediateWithYValue(MutableVector self, MutableVector other, double y) {
        double d0 = other.getX() - self.getX();
        double d1 = other.getY() - self.getY();
        double d2 = other.getZ() - self.getZ();
     ...
     * passed in vector, or null if not possible.
     */
    public static MutableVector getIntermediateWithZValue(MutableVector self, MutableVector other, double z) {
        double d0 = other.getX() - self.getX();
        double d1 = other.getY() - self.getY();
        double d2 = other.getZ() - self.getZ();
     ...
     */
    private static boolean isVecInYZ(SimpleCollisionBox self, MutableVector vec) {
     ...
     */
    private static boolean isVecInXZ(SimpleCollisionBox self, MutableVector vec) {
     ...
     */
    private static boolean isVecInXY(SimpleCollisionBox self, MutableVector vec) {
    ...
    public static MutableVector getLook(GrimPlayer player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            ...
            return new MutableVector(f1 * f2, f3, f * f2);
        } else {
            ...
            return new MutableVector(f3 * f4, -f5, (double) (f2 * f4));
        }
    }

    public static boolean isVecInside(SimpleCollisionBox self, MutableVector vec) {
    ...
    public static double getMinReachToBox(GrimPlayer player, SimpleCollisionBox targetBox) {
        ...
        for (double eyes : player.getPossibleEyeHeights()) {
            ...
            MutableVector from = new MutableVector(player.x, player.y + eyes, player.z);
            MutableVector closestPoint = VectorUtils.cutBoxToVector(from, targetBox);
