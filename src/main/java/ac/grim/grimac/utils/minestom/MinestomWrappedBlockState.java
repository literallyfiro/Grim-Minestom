...
public class MinestomWrappedBlockState {
    private static final MinestomWrappedBlockState AIR = new MinestomWrappedBlockState(Block.AIR, new EnumMap<>(StateValue.class), 0);
    private static final Map<String, MinestomWrappedBlockState> BY_STRING = new HashMap<>();
    private static final Map<Integer, MinestomWrappedBlockState> BY_ID = new HashMap<>();
    private static final Map<MinestomWrappedBlockState, String> INTO_STRING = new HashMap<>();
    private static final Map<MinestomWrappedBlockState, Integer> INTO_ID = new HashMap<>();
    private static final Map<Block, MinestomWrappedBlockState> DEFAULT_STATES = new HashMap<>();
    ...
    static {
        ...
        Map<CompoundBinaryTag, Map.Entry<Map<StateValue, Object>, String>> cache = new HashMap<>(6160, 70);

        loadModernMinestom(cache);

        cache.clear();
        cache = null;
    }

    private static Block getByName(String name) {
        return Block.fromNamespaceId(name);
    }

    private static void loadModernMinestom(Map<CompoundBinaryTag, Map.Entry<Map<StateValue, Object>, String>> cache) {
        Map<Integer, MinestomWrappedBlockState> stateByIdMap = new HashMap<>();
        Map<MinestomWrappedBlockState, Integer> stateToIdMap = new HashMap<>();
        Map<String, MinestomWrappedBlockState> stateByStringMap = new HashMap<>();
        Map<MinestomWrappedBlockState, String> stateToStringMap = new HashMap<>();
        Map<Block, MinestomWrappedBlockState> stateTypeToBlockStateMap = new HashMap<>();

        int id = 0;
        // This thing makes me want to throw up so fucking bad oh my fucking god
        try {
            Class<?> cls = Class.forName("net.minestom.server.instance.block.Blocks");
            Field[] fieldList = cls.getDeclaredFields();
            for (Field fld : fieldList) {
                // fld is a block
                fld.setAccessible(true);
                // WHAT IS THIS
                Block block = (Block) fld.get(null);
                @Unmodifiable Map<String, String> properties = block.properties();
                if (properties.isEmpty()) {
                    MinestomWrappedBlockState state = new MinestomWrappedBlockState(block, new EnumMap<>(StateValue.class), id);
                    stateByIdMap.put(id, state);
                    stateToIdMap.put(state, id);
                    stateByStringMap.put(propertiesToString(block), state);
                    stateToStringMap.put(state, propertiesToString(block));
                    stateTypeToBlockStateMap.put(block, state);
                    id++;
                } else {
                    for (@NotNull Block possibleState : block.possibleStates()) {
                        Map<StateValue, Object> data = new EnumMap<>(StateValue.class);
                        for (Map.Entry<String, String> stringStringEntry : possibleState.properties().entrySet()) {
                            ...
                            try {
                                ...
                                data.put(stateValue, stateValue.getParser().apply(value.toUpperCase(Locale.ROOT)));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }

                        MinestomWrappedBlockState state = new MinestomWrappedBlockState(possibleState, data, id);
                        stateByIdMap.put(id, state);
                        stateToIdMap.put(state, id);
                        stateByStringMap.put(propertiesToString(possibleState), state);
                        stateToStringMap.put(state, propertiesToString(possibleState));
                        stateTypeToBlockStateMap.put(possibleState, state);
                        id++;
                    }
                }
            }
        ...
        BY_ID.putAll(stateByIdMap);
        INTO_ID.putAll(stateToIdMap);
        BY_STRING.putAll(stateByStringMap);
        INTO_STRING.putAll(stateToStringMap);
        DEFAULT_STATES.putAll(stateTypeToBlockStateMap);
    }

    private int globalID;
    private Block type;
    private Map<StateValue, Object> data;
    private boolean hasClonedData = false;
    ...
    public static MinestomWrappedBlockState getByGlobalId(int globalID, boolean clone) {
        ...
        final MinestomWrappedBlockState state = BY_ID.getOrDefault(globalID, AIR);
        return clone ? state.clone() : state;
    }

    @NotNull
    public static MinestomWrappedBlockState getByString(String string) {
        return getByString(string, true);
    }

    @NotNull
    public static MinestomWrappedBlockState getByString(String string, boolean clone) {
        final MinestomWrappedBlockState state = BY_STRING.getOrDefault(string, AIR);
        return clone ? state.clone() : state;
    }

    @NotNull
    public static MinestomWrappedBlockState getDefaultState(Block type) {
        return getDefaultState(type, true);
    }

    @NotNull
    public static MinestomWrappedBlockState getDefaultState(Block type, boolean clone) {
        if (type == Block.AIR) return AIR;
        MinestomWrappedBlockState state = DEFAULT_STATES.get(type);
        if (state == null) {
            System.out.println("Default state for " + type.name() + " is null. Returning AIR");
            return AIR;
        }
        return clone ? state.clone() : state;
    }
    ...
    @NotNull
    public static String propertiesToString(Block type) {
        Map<String, String> properties = type.properties();
    ...
    public MinestomWrappedBlockState clone() {
        return new MinestomWrappedBlockState(type, data, globalID);
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof MinestomWrappedBlockState that)) return false;
...
//            MinestomWrappedBlockState blockState = BY_ID.getOrDefault(oldGlobalID, AIR).clone();
