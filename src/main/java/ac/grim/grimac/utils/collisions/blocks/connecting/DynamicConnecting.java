...
public class DynamicConnecting {
    ...
    public boolean connectsTo(GrimPlayer player, ClientVersion v, int currX, int currY, int currZ, ac.grim.grimac.utils.minestom.BlockFace direction) {
        MinestomWrappedBlockState targetBlock = player.compensatedWorld.getWrappedBlockStateAt(currX + direction.getModX(), currY + direction.getModY(), currZ + direction.getModZ());
