...
public enum BlockPlaceResult {
    ...
    ANVIL((player, place) -> {
        MinestomWrappedBlockState data = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    SNOW((player, place) -> {
        Point against = place.getPlacedAgainstBlockLocation();
        ...
        if (!BlockTags.ICE.contains(below.getType()) && below.getType() != Block.BARRIER) {
            ...
            if (set) {
                if (blockState.getType() == Block.SNOW) {
                    MinestomWrappedBlockState snow = MinestomWrappedBlockState.getDefaultState(Block.SNOW);
    ...
    SLAB((player, place) -> {
        ...
        MinestomWrappedBlockState slabData = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    STAIRS((player, place) -> {
        BlockFace direction = place.getDirection();
        MinestomWrappedBlockState stair = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    END_ROD((player, place) -> {
        MinestomWrappedBlockState endRod = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    LADDER((player, place) -> {
        ...
        for (BlockFace face : place.getNearestPlacingDirections()) {
            ...
            if (BlockFaceHelper.isFaceHorizontal(face) && place.isFullFace(face)) {
                MinestomWrappedBlockState ladder = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    AMETHYST_CLUSTER((player, place) -> {
        MinestomWrappedBlockState amethyst = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    BAMBOO((player, place) -> {
        Point clicked = place.getPlacedAgainstBlockLocation();
        if (player.compensatedWorld.getFluidLevelAt(clicked.blockX(), clicked.blockY(), clicked.blockZ()) > 0) return;
    ...
    BELL((player, place) -> {
        BlockFace direction = place.getDirection();
        MinestomWrappedBlockState bell = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    CANDLE((player, place) -> {
        MinestomWrappedBlockState existing = place.getExistingBlockData();
        MinestomWrappedBlockState candle = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    SEA_PICKLE((player, place) -> {
        ...
        if (existing.getType() == Block.SEA_PICKLE) {
            ...
            existing = MinestomWrappedBlockState.getDefaultState(Block.SEA_PICKLE);
        }
    ...
    CHAIN((player, place) -> {
        MinestomWrappedBlockState chain = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    DIRT_PATH((player, place) -> {
        MinestomWrappedBlockState state = place.getDirectionalState(BlockFace.UP);
    ...
    HOPPER((player, place) -> {
        ...
        MinestomWrappedBlockState hopper = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    LANTERN((player, place) -> {
        for (BlockFace face : place.getNearestPlacingDirections()) {
            ...
            MinestomWrappedBlockState lantern = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    POINTED_DRIPSTONE((player, place) -> {
        ...
        MinestomWrappedBlockState toPlace = MinestomWrappedBlockState.getDefaultState(Block.POINTED_DRIPSTONE);
        ...
        Point placedPos = place.getPlacedBlockPos();
        Dripstone.update(player, toPlace, placedPos.blockX(), placedPos.blockY(), placedPos.blockZ(), place.isSecondaryUse());
    ...
    CACTUS((player, place) -> {
        ...
        if (place.isOn(Block.CACTUS, Block.SAND, Block.RED_SAND) && !place.isLava(BlockFace.UP)) {
            place.set();
        }
    }, Material.CACTUS),
    ...
    PISTON_BASE((player, place) -> {
        MinestomWrappedBlockState piston = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    SUGARCANE((player, place) -> {
        ...
        if (place.isOnDirt() || place.isOn(Block.SAND, Block.RED_SAND)) {
            ...
            for (BlockFace direction : place.getHorizontalFaces()) {
                Point toSearchPos = pos;
                toSearchPos = toSearchPos.withX(toSearchPos.blockY() + direction.getModX());
                toSearchPos = toSearchPos.withZ(toSearchPos.blockY() + direction.getModZ());
    ...
    WATERLILY((player, place) -> {
        MinestomWrappedBlockState below = place.getDirectionalState(BlockFace.DOWN);
    ...
    TORCH_OR_HEAD((player, place) -> {
        ...
        if (isTorch) {
            dir = MinestomWrappedBlockState.getDefaultState(Block.WALL_TORCH);
        } else if (place.getMaterial().namespace().key().asString().contains("head") || place.getMaterial().namespace().key().asString().contains("skull")) {
            dir = MinestomWrappedBlockState.getDefaultState(Block.PLAYER_WALL_HEAD);
        } else {
            dir = MinestomWrappedBlockState.getDefaultState(Block.OAK_WALL_SIGN);
        }
    ...
    MULTI_FACE_BLOCK((player, place) -> {
        ...
        if (multiFace.getType() != placedType) {
            multiFace = MinestomWrappedBlockState.getDefaultState(placedType);
        }
    ...
    GRINDSTONE((player, place) -> { // Grindstones do not have special survivability requirements
        MinestomWrappedBlockState stone = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
        if (place.isFaceVertical()) {
        ...
        for (BlockFace face : place.getNearestPlacingDirections()) {
            if (place.isSolidBlocking(face) && face != BlockFace.UP) {
                if (BlockFaceHelper.isFaceHorizontal(face)) {
                    ...
                    // If it's a torch, create a wall torch
                    // Otherwise, it's going to be a head.  The type of this head also doesn't matter.
                    MinestomWrappedBlockState dir = MinestomWrappedBlockState.getDefaultState(Block.BLACK_WALL_BANNER);
                    dir.setFacing(face.getOppositeFace());
                    place.set(dir);
                } else {
                    place.set(place.getMaterial());
    ...
    BIG_DRIPLEAF((player, place) -> {
        MinestomWrappedBlockState existing = place.getDirectionalState(BlockFace.DOWN);
        if (place.isFullFace(BlockFace.DOWN) || existing.getType() == Block.BIG_DRIPLEAF || existing.getType() == Block.BIG_DRIPLEAF_STEM) {
            place.set(place.getMaterial());
        }
    ...
    SMALL_DRIPLEAF((player, place) -> {
        MinestomWrappedBlockState existing = place.getDirectionalState(BlockFace.DOWN);
        if (place.isBlockFaceOpen(BlockFace.UP) && BlockTags.SMALL_DRIPLEAF_PLACEABLE.contains(existing.getType()) || (place.isInWater() && (place.isOnDirt() || existing.getType() == Block.FARMLAND))) {
            place.set(place.getMaterial());
        }
    ...
    SEAGRASS((player, place) -> {
        MinestomWrappedBlockState existing = place.getDirectionalState(BlockFace.DOWN);
        if (place.isInWater() && place.isFullFace(BlockFace.DOWN) && existing.getType() != Block.MAGMA_BLOCK) {
            place.set(place.getMaterial());
        }
    }, Material.SEAGRASS),
            ...
            if (face != BlockFace.UP) {
                ...
                if (BlockFaceHelper.isFaceHorizontal(face)) {
                    if (canPlace) { // center requires nothing (head), full face (torch), or solid (sign)
                        MinestomWrappedBlockState coralFan = MinestomWrappedBlockState.getDefaultState(Block.FIRE_CORAL_WALL_FAN);
    ...
    KELP((player, place) -> {
        Block below = place.getDirectionalState(BlockFace.DOWN).getType();
        MinestomWrappedBlockState existing = place.getExistingBlockData();
        ...
        if (below != Block.MAGMA_BLOCK && (place.isFullFace(BlockFace.DOWN) || below == Block.KELP || below == Block.KELP_PLANT) && fluidLevel >= 8 / 9d) {
            place.set(place.getMaterial());
        }
    }, Material.KELP),

    CAVE_VINE((player, place) -> {
        Block below = place.getDirectionalState(BlockFace.UP).getType();
        if (place.isFullFace(BlockFace.DOWN) || below == Block.CAVE_VINES || below == Block.CAVE_VINES_PLANT) {
            place.set(place.getMaterial());
        }
    ...
    WEEPING_VINE((player, place) -> {
        Block below = place.getDirectionalState(BlockFace.UP).getType();
        if (place.isFullFace(BlockFace.UP) || below == Block.WEEPING_VINES || below == Block.WEEPING_VINES_PLANT) {
            place.set(place.getMaterial());
        }
    ...
    TWISTED_VINE((player, place) -> {
        Block below = place.getDirectionalState(BlockFace.DOWN).getType();
        if (place.isFullFace(BlockFace.DOWN) || below == Block.TWISTING_VINES || below == Block.TWISTING_VINES_PLANT) {
            place.set(place.getMaterial());
        }
    ...
    LECTERN((player, place) -> {
        MinestomWrappedBlockState lectern = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
        lectern.setFacing(place.getPlayerFacing().getOppositeFace());
        place.set(lectern);
    }, Material.LECTERN),

    FENCE_GATE((player, place) -> {
        MinestomWrappedBlockState gate = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    TRAPDOOR((player, place) -> {
        MinestomWrappedBlockState door = MinestomWrappedBlockState.getDefaultState(place.getMaterial());
    ...
    DOOR((player, place) -> {
        if (place.isFullFace(BlockFace.DOWN) && place.isBlockFaceOpen(BlockFace.UP)) {
            ...
            MutableVector aboveCCWPos = place.getClickedLocation().add(new MutableVector(ccw.getModX(), ccw.getModY(), ccw.getModZ())).add(new MutableVector(0, 1, 0));
            ...
            MutableVector aboveCWPos = place.getClickedLocation().add(new MutableVector(cw.getModX(), cw.getModY(), cw.getModZ())).add(new MutableVector(0, 1, 0));
...
//            }
    ...
    SCAFFOLDING((player, place) -> {
        ...
        if (place.getPlacedAgainstMaterial() == Block.SCAFFOLDING) {
            ...
            int i = 0;
            Point starting = new Vec(place.getPlacedAgainstBlockLocation().blockX() + direction.getModX(), place.getPlacedAgainstBlockLocation().blockY() + direction.getModY(), place.getPlacedAgainstBlockLocation().blockZ() + direction.getModZ());
            while (i < 7) {
                ...
                starting = new Vec(starting.blockX() + direction.getModX(), starting.blockY() + direction.getModY(), starting.blockZ() + direction.getModZ());
        ...
        MinestomWrappedBlockState scaffolding = MinestomWrappedBlockState.getDefaultState(Block.SCAFFOLDING);
    ...
    DOUBLE_PLANT((player, place) -> {
        if (place.isBlockFaceOpen(BlockFace.UP) && place.isOnDirt() || place.isOn(Block.FARMLAND)) {
            place.set();
