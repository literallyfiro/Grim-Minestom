...
public class ConsumesBlockPlace {
    public static boolean consumesPlace(GrimPlayer player, MinestomWrappedBlockState state, BlockPlace place) {
        ...
        if (BlockTags.CANDLE_CAKES.contains(state.getType())) {
            MinestomWrappedBlockState cake = MinestomWrappedBlockState.getDefaultState(Block.CAKE);
            cake.setBites(1);
        ...
        if (state.getType() == Block.CAKE) {
            if (state.getBites() == 0 && BlockTags.CANDLES.contains(place.getMaterial())) {
                player.compensatedWorld.updateBlock(place.getPlacedAgainstBlockLocation(), MinestomWrappedBlockState.getDefaultState(Block.CANDLE_CAKE));
                return true;
            }
            ...
            if (player.gamemode == GameMode.CREATIVE || (player.food < 20)) {
                if (state.getBites() != 6) {
                    ...
                    player.compensatedWorld.updateBlock(place.getPlacedAgainstBlockLocation(), MinestomWrappedBlockState.getDefaultState(Block.AIR));
                }
                return true;
            }
    ...
    private static boolean goodBellHit(MinestomWrappedBlockState bell, BlockPlace place) {
        ...
        return place.getHitData() != null && isProperHit(bell, direction, place.getHitData().getRelativeBlockHitLocation().y());
    }
