...
public class Reach extends Check implements PacketCheck {
    ...
    private final Map<Integer, Point> playerAttackQueue = new HashMap<>();
    ...
    public void onPacketReceive(final PlayerPacketEvent event) {
        if (!player.disableGrim && event.getPacket() instanceof ClientInteractEntityPacket action) {
            ...
            if (!tooManyAttacks) {
                playerAttackQueue.put(action.targetId(), new Vec(player.x, player.y, player.z)); // Queue for next tick for very precise check
            }
    ...
    private boolean isKnownInvalid(PacketEntity reachEntity) {
        ...
        if (cancelBuffer != 0) {
            return checkReach(reachEntity, new Vec(player.x, player.y, player.z), true) != null; // If they flagged
        } else {
            SimpleCollisionBox targetBox = reachEntity.getPossibleCollisionBoxes();
            if (reachEntity.getType() == EntityType.END_CRYSTAL) {
                targetBox = new SimpleCollisionBox(reachEntity.trackedServerPosition.getPos().sub(1, 0, 1), reachEntity.trackedServerPosition.getPos().add(1, 2, 1));
            }
    ...
    private void tickBetterReachCheckWithAngle() {
        for (Map.Entry<Integer, Point> attack : playerAttackQueue.entrySet()) {
    ...
    private String checkReach(PacketEntity reachEntity, Point from, boolean isPrediction) {
        SimpleCollisionBox targetBox = reachEntity.getPossibleCollisionBoxes();
        ...
        for (MutableVector lookVec : possibleLookDirs) {
            for (double eye : player.getPossibleEyeHeights()) {
                MutableVector eyePos = new MutableVector(from.x(), from.y() + eye, from.z());
