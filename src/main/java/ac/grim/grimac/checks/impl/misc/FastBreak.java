...
public class FastBreak extends Check implements PacketCheck {
    ...
    public void onPacketReceive(PlayerPacketEvent event) {
        ...
        if (event.getPacket() instanceof ClientPlayerDiggingPacket digging) {
            final Point blockPosition = digging.blockPosition();
            ...
            if (digging.status() == ClientPlayerDiggingPacket.Status.FINISHED_DIGGING && targetBlock != null) {
                ...
                if (blockBreakBalance > 1000) { // If more than a second of advantage
                    ...
                    if (bukkitPlayer.getPosition().distance(blockPosition.blockX(), blockPosition.blockY(), blockPosition.blockZ()) < 64) {
                        final int chunkX = blockPosition.blockX() >> 4;
                        final int chunkZ = blockPosition.blockZ() >> 4;
                        ...
                        Block block = chunk.getBlock(blockPosition.blockX() & 15, blockPosition.blockY(), blockPosition.blockZ() & 15);
...
//                        }
                        ...
                        player.bukkitPlayer.sendPacket(new BlockChangePacket(blockPosition, block));
