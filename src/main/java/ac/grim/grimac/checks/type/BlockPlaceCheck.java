...
public class BlockPlaceCheck extends Check implements RotationCheck, PostPredictionCheck {
    ...
    protected SimpleCollisionBox getCombinedBox(final BlockPlace place) {
        ...
        CollisionBox placedOn = HitboxData.getBlockHitbox(player, place.getMaterial(), player.getClientVersion(), player.compensatedWorld.getWrappedBlockStateAt(clicked), clicked.blockX(), clicked.blockY(), clicked.blockZ());

        List<SimpleCollisionBox> boxes = new ArrayList<>();
        placedOn.downCast(boxes);

        SimpleCollisionBox combined = new SimpleCollisionBox(clicked.x(), clicked.y(), clicked.z());
        ...
        if (weirdBoxes.contains(place.getPlacedAgainstMaterial())) {
            ...
            combined = new SimpleCollisionBox(clicked.x() + 1, clicked.y() + 1, clicked.z() + 1, clicked.x(), clicked.y(), clicked.z());
        }
