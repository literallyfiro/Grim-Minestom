...
public class CheckManagerListener {
    ...
    public static HitData traverseBlocks(GrimPlayer player, Vector3d start, Vector3d end, BiFunction<MinestomWrappedBlockState, Point, HitData> predicate) {
        ...
        HitData apply = predicate.apply(state, new Vec(floorStartX, floorStartY, floorStartZ));
        ...
        while (d12 <= 1.0D || d13 <= 1.0D || d14 <= 1.0D) {
            ...
            apply = predicate.apply(state, new Vec(floorStartX, floorStartY, floorStartZ));
    ...
    private static void placeWaterLavaSnowBucket(GrimPlayer player, ModifiableItemStack held, Block toPlace, Player.Hand hand) {
        ...
        if (data != null) {
            System.out.println("placeWaterLavaSnowBucket, " + held.getType() + ", " + toPlace + ", " + hand);
    ...
    private static void handleUseItem(GrimPlayer player, ModifiableItemStack placedWith, Player.Hand hand) {
        ...
        Block toBucketMat = Materials.transformBucketMaterial(placedWith.getType());
        if (toBucketMat != null) {
    ...
    private static void handleBlockPlaceOrUseItem(ClientPacket packet, GrimPlayer player) {
        ...
        if (packet instanceof ClientPlayerBlockPlacementPacket place) {
            ...
            if ((!player.isSneaking || onlyAir) && place.hand() == Player.Hand.MAIN) {
                Point blockPosition = place.blockPosition();
                BlockFace face = BlockFace.fromMinestom(place.blockFace());
                System.out.println("ClientPlayerBlockPlacementPacket, " + placedWith.getType() + ", " + blockPosition + ", " + face);
                ...
                if ((player.getClientVersion().isOlderThan(ClientVersion.V_1_8) && (placedAgainst == Block.IRON_TRAPDOOR || placedAgainst == Block.IRON_DOOR))
                    ...
                    Point location = blockPlace.getPlacedAgainstBlockLocation();
                    player.compensatedWorld.tickOpenable(location.blockX(), location.blockY(), location.blockZ());
                    return;
                }
        ...
        if (packet instanceof ClientPlayerBlockPlacementPacket place) {
            ...
            System.out.println("ClientPlayerBlockPlacementPacket11, " + placedWith.getType() + ", " + blockPosition + ", " + face);
            BlockPlace blockPlace = new BlockPlace(player, place.hand(), blockPosition, place.blockFace().ordinal(), face, placedWith, getNearestHitResult(player, null, true));
            ...
            if (placedWith.getType() != null || placedWith.getType() == Material.FIRE_CHARGE) {
...
//            flying.setLocation(new Location(player.filterMojangStupidityOnMojangStupidity.getX(), player.filterMojangStupidityOnMojangStupidity.getY(), player.filterMojangStupidityOnMojangStupidity.getZ(), location.yaw(), location.pitch()));
    ...
    public void onPacketReceive(PlayerPacketEvent event) {
        ...
        if (event.getPacket() instanceof ClientPlayerDiggingPacket dig) {
            ...
            if (dig.status() == ClientPlayerDiggingPacket.Status.STARTED_DIGGING && !event.isCancelled()) {
                double damage = BlockBreakSpeed.getBlockDamage(player, dig.blockPosition());
                ...
                if (damage >= 1) {
                    ...
                    if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_13) && Materials.isWaterSource(player.getClientVersion(), block)) {
                        ...
                        player.compensatedWorld.updateBlock(dig.blockPosition(), MinestomWrappedBlockState.getDefaultState(Block.WATER));
                    } else {
        ...
        if (event.getPacket() instanceof ClientPlayerBlockPlacementPacket packet) {
            ...
            if (packet.hand() == Player.Hand.OFF) {
                System.out.println("ClientPlayerBlockPlacementPacket2, offhand");
                placedWith = player.getInventory().getOffHand();
            }

            // This is the use item packet
            // Anti-air place
            BlockFace face = BlockFace.fromMinestom(packet.blockFace());
            System.out.println("ClientPlayerBlockPlacementPacket2, " + placedWith.getType() + ", " + packet.blockPosition() + ", " + face);
            BlockPlace blockPlace = new BlockPlace(player, packet.hand(), packet.blockPosition(), packet.blockFace().ordinal(), face, placedWith, getNearestHitResult(player, null, true));
            blockPlace.setCursor(new Vec(packet.cursorPositionX(), packet.cursorPositionY(), packet.cursorPositionZ()));
            ...
            if (player.getClientVersion().isOlderThan(ClientVersion.V_1_11)) {
                ...
                if (packet.cursorPositionX() * 15 % 1 == 0 && packet.cursorPositionY() * 15 % 1 == 0 && packet.cursorPositionZ() * 15 % 1 == 0) {
                    ...
                    blockPlace.setCursor(new Vec(trueByteX / 16f, trueByteY / 16f, trueByteZ / 16f));
                }
            }
            ...
            if (event.isCancelled() || blockPlace.isCancelled() || player.getSetbackTeleportUtil().shouldBlockMovement()) { // The player tried placing blocks in air/water
                ...
                Point facePos = new Vec(packet.blockPosition().blockX() + face.getModX(),
                        packet.blockPosition().blockY() + face.getModY(),
                        packet.blockPosition().blockZ() + face.getModZ());
    ...
    private static void placeBucket(GrimPlayer player, Player.Hand hand) {
        ...
        if (data != null) {
            System.out.println("placeBucket, empty");
            ...
            if (!placed && !player.compensatedWorld.isWaterSourceBlock(data.getPosition().blockX(), data.getPosition().blockY(), data.getPosition().blockZ()))
                return;
        ...
        if (player.gamemode != GameMode.CREATIVE) {
            if (hand == Player.Hand.MAIN) {
                if (player.getInventory().getHeldItem().getAmount() == 1) {
                    ...
                    System.out.println("setplayeritem");
    ...
    private static void placeLilypad(GrimPlayer player, Player.Hand hand) {
        ...
        if (data != null) {
            ...
            if (player.compensatedWorld.getFluidLevelAt(data.getPosition().blockX(), data.getPosition().blockY() + 1, data.getPosition().blockZ()) > 0)
                return;

            System.out.println("placeLilypad, empty");
            BlockPlace blockPlace = new BlockPlace(player, hand, data.getPosition(), data.getClosestDirection().ordinal(), data.getClosestDirection(), ModifiableItemStack.EMPTY, data);
            blockPlace.setReplaceClicked(false); // Not possible with use item

            // We checked for a full fluid block below here.
            if (player.compensatedWorld.getWaterFluidLevelAt(data.getPosition().blockX(), data.getPosition().blockY(), data.getPosition().blockZ()) > 0
                    || data.getState().getType() == Block.ICE || data.getState().getType() == Block.FROSTED_ICE) {
                Point pos = data.getPosition();
                pos = pos.add(0, 1, 0);

                blockPlace.set(pos, MinestomWrappedBlockState.getDefaultState(Block.LILY_PAD));
    ...
    private static HitData getNearestHitResult(GrimPlayer player, Block heldItem, boolean sourcesHaveHitbox) {
        ...
        return traverseBlocks(player, startingPos, endPos, (block, vector3i) -> {
            CollisionBox data = HitboxData.getBlockHitbox(player, heldItem, player.getClientVersion(), block, vector3i.blockX(), vector3i.blockY(), vector3i.blockZ());
            ...
            if (sourcesHaveHitbox &&
                    (player.compensatedWorld.isWaterSourceBlock(vector3i.blockX(), vector3i.blockY(), vector3i.blockZ())
                            || player.compensatedWorld.getLavaFluidLevelAt(vector3i.blockX(), vector3i.blockY(), vector3i.blockZ()) == (8 / 9f))) {
                double waterHeight = player.compensatedWorld.getFluidLevelAt(vector3i.blockX(), vector3i.blockY(), vector3i.blockZ());
                SimpleCollisionBox box = new SimpleCollisionBox(vector3i.blockX(), vector3i.blockY(), vector3i.blockZ(), vector3i.blockX() + 1, vector3i.blockY() + waterHeight, vector3i.blockZ() + 1);
